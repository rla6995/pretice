    ...
    for idx, var1 in enumerate(count_list):
        ...
        for idc,var2 in enumerate(tds):
            ...
            try:
                ...
                pass
            # 추출 데이터들 목록에 넣기
            Modern_Softwood.append(sub_wood)
            # 서브 목록 초기화
            sub_wood=[]
            # 목록으로 돌아가기
            wd.back()
...
for var4 in range(len(c)):
    if type(c[var4])==list:
        c[var4]=" ".join(c[var4])
# 데이터프레임으로 변환
MS_df=pd.DataFrame(a,columns=["name1","name2","name3","name4"])
MS_number=pd.Series(b_re2,name="number")
MS_reference=pd.Series(c,name="reference")
MS_df["number"]=MS_number
MS_df["reference"]=MS_reference
# 액셀 파일로 변환 저장
MS_df.to_excel('MS_all.xlsx', index=True)
